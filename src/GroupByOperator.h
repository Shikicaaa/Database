...
public:
    GroupByOperator(std::unique_ptr<Operator> child, const std::vector<std::string>& group_by_columns, const std::vector<SelectItem>& select_items)
    : child_(std::move(child)), group_by_columns_(group_by_columns), select_items_(select_items) {}
